...
%type <mat> initializer_row_list initializer_row
%type <stat>  statement
	labeled_statement 
	compound_statement
	block_item_list
...
initializer
	: BRO initializer_row_list BRC {
		...
		//cout << "error here1\n";
		string str = "{";
		for(int i=0; i< $2->initmat.size();i++){
...
initializer_row_list : initializer_row {
	...
	for(int i=0; i < $1->initmat.size(); i++){
		//cout << i << endl;
		$$->initmat.push_back($1->initmat[i]);
	}
...
|initializer_row_list SECOL initializer_row {
	...
	for(int i=0; i < $3->initmat.size(); i++){
		//cout << i << endl;
		$$->initmat.push_back($3->initmat[i]);
	}
	//$$->initmat.push_back(";");
...
selection_statement
	: SWITCH CO expression N CC M statement N                                           /* Skipped */
	{	
		$3->isswitch= true;
	...
	| IF CO expression N CC M statement N ELSE M statement {  
