...
ostream& operator<<(ostream& os, const quad* q) {
	...
	switch(op) {
		...
		case ADD: {
			if (isInteger(arg2))
				os << "addl \t$" << atoi(arg2.c_str()) << ", " << table->ar[arg1] << "(%rbp)";
			else {
				os << "movl \t" << table->ar[arg1] << "(%rbp), " << "%rax" << endl;
					os << "\tmovl \t" << table->ar[arg2] << "(%rbp), " << "%rdx" << endl;
				os << "\taddl \t%rdx, %rax\n";
				os << "\tmovl \t%rax, " << table->ar[result] << "(%rbp)";
			}
			break;
		}
		case SUB:{
		...
		case MULT: {
			os << "movl \t" << table->ar[arg1] << "(%rbp), " << "%rax" << endl;
			if (isInteger(arg2))
				os << "\timull \t$" << atoi(arg2.c_str()) << ", " << "%rax" << endl;
			else
				os << "\timull \t" << table->ar[arg2] << "(%rbp), " << "%rax" << endl;
			os << "\tmovl \t%rax, " << table->ar[result] << "(%rbp)";
			break;			
		}
		case DIVIDE: {
			os << "movl \t" << table->ar[arg1] << "(%rbp), " << "%rax" << endl;
			os << "\tcltd" << endl;
			os << "\tidivl \t" << table->ar[arg2] << "(%rbp)" << endl;
			os << "\tmovl \t%rax, " << table->ar[result] << "(%rbp)";
			break;			
		}
		...
		case EQUAL:	{
			if(isInteger(arg1))
				os << "movl\t$" << atoi(arg1.c_str()) << ", " << "%rax" << endl;
			else
				os << "movl\t" << table->ar[arg1] << "(%rbp), " << "%rax" << endl;
			os << "\tmovl \t%rax, " << table->ar[result] << "(%rbp)";
			break;
		...
		case EQUALSTR:	{
			os << "movl \t$.LC" << arg1 << ", " << table->ar[result] << "(%rbp)";
			break;
		}
		case EQUALCHAR:	{
			os << "movb\t$" << atoi(arg1.c_str()) << ", " << table->ar[result] << "(%rbp)";
			break;
		}					
		// Relational Operations
		case EQOP: {
			os << "movl\t" << table->ar[arg1] << "(%rbp), %rax\n";
			os << "\tcmpl\t" << table->ar[arg2] << "(%rbp), %rax\n";
		...
		case PTRL: {
			os << "movl\t" << table->ar[result] << "(%rbp), %rax\n";
			os << "\tmovl\t" << table->ar[arg1] << "(%rbp), %rdx\n";
			os << "\tmovl\t%rdx, (%rax)";
			break;
		} 			
		case UMINUS:{
			os << "negl\t" << table->ar[arg1] << "(%rbp)";
			break;
		}
		case BNOT:			os << result 	<< " = ~" << arg1;				break;
		case LNOT:			os << result 	<< " = !" << arg1;				break;
		case ARRR: {
			os << "movl\t" << table->ar[arg2] << "(%rbp), "<<"%rax" << endl;
			os << "\tmovl\t" << table->ar[arg1] << "(%rbp,%rax,1), "<<"%rax" << endl;
			os << "\tmovl \t%rax, " <<  table->ar[result] << "(%rbp)";
			break;
		}	 			
		case ARRL: {
			os << "movl\t" << table->ar[arg1] << "(%rbp), "<<"%rax" << endl;
			os << "\tmovl\t" << table->ar[arg2] << "(%rbp), "<<"%rdx" << endl;
			os << "\tmovl\t" << "%rdx, " << table->ar[result] << "(%rbp,%rax,1)";
			break;
		}	 
		case _RETURN: {
			if(result!="") os << "movl\t" << table->ar[result] << "(%rbp), "<<"%rax";
			else os << "nop";
			break;
		}
		case PARAM: {
		...
		case CALL: {
			...
			for (list <sym>::iterator it = t->table.begin(); it!=t->table.end(); it++) {
				...
				if (it->category== "param") {
					os << "movl \t" << table->ar[params[i]] << "(%rbp), " << "%rax" << endl;
					os << "\tmovl \t%rax, " << (t->ar[it->name]-8 )<< "(%rsp)\n\t";
				}
				else break;
			}
			params.clear();
...
void genasm() {
	...
	asmfile << 	"\t.ident\t	\"Compiled by Pankaj Dhurve\"\n";
...
int  main (int ac, char* av[]) {
	...
	try {
		...
		//if (vm.count("ofilename")) ofilepath = vm["ofilename"].as< vector<string> >()[0];
		 ofilepath = removeExtension(inputfile) = "quad.log";

		//if (vm.count("sfilepath")) sfilepath = vm["sfilepath"].as< vector<string> >()[0];
		 sfilepath = removeExtension(inputfile) = "asm.s";
