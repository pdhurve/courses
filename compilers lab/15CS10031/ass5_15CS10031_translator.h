...
/** Attributes/Global for Boolean unaryession***/
...
struct unary {
	bool isswitch;
	lint nextlist;
	lint falselist;				// Falselist valid for boolean unaryessions
	...
	type_e cat;
	bool isbool;				// Boolean variable that stores if the unaryession is bool
};

struct matrixinit{
	sym* symp;
	vector<string> initmat;
};
...
unary* convertfrombool (unary*);			// convert bool to unaryession
unary* convert2bool (unary*);				// convert any unaryession to bool
